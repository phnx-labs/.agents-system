...
### Fixed
- **`merge-guard.sh` no longer false-blocks commands that merely *mention* an admin-bypass merge in body/message text.** The guard did a naive substring match over the entire command string, so a `gh pr create` / `git commit` whose `--body` or `-m` text documented the guard (as this repo's own rules do) was blocked as if it were a bypass merge — it fired on a `gh pr create` during PR #40. The guard now removes only genuinely **inert** text before matching (via `perl`, with a safe raw-match fallback if `perl` is absent): values of documentation flags (`--body`/`-b`/`--title`/`-t`/`-m`/`--message`/…) and heredoc bodies feeding a non-shell command. Anything that actually **executes** is kept visible — a doc-flag value containing a command substitution (`$(…)`/backtick), a heredoc piped into `sh`/`bash`/`eval`, and `sh -c '…'` / bare invocations — so a real bypass merge is still caught. Never fails open on a genuine bypass.
  - Added `rules/subrules/gh-merge-guard/merge-guard_test.sh` — 16 cases over the real script and real stdin JSON: genuine bypass merges block (plain, chained `create && merge`, `sh -c` both quote styles, `$(…)` in `-m`, backtick, shell-fed heredoc); legit merges and unrelated commands pass; and body / commit-message / cat-heredoc text mentioning the tokens passes.
