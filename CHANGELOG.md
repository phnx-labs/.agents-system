...
### Fixed
- **`merge-guard.sh` no longer false-blocks commands that merely *mention* an admin-bypass merge in body/message text.** The guard did a naive substring match over the entire command string, so a `gh pr create` / `git commit` whose `--body` or `-m` text documented the guard (as this repo's own rules do) was blocked as if it were a bypass merge — it fired on a `gh pr create` during PR #40. The guard now removes only genuinely **inert** text before matching (via `perl`, with a safe raw-match fallback if `perl` is absent): values of documentation flags (`--body`/`-b`/`--title`/`-t`/`-m`/`--message`/…) whose content is plain text, and heredoc bodies that feed a known **data-sink** command (`cat`/`tee`/`gh`/`git`). Everything else is kept visible so a real bypass merge is still caught — a doc-flag value containing a command substitution (`$(…)`/backtick), `sh -c '…'` / bare invocations, and any heredoc fed to an interpreter (`sh`/`bash`/`. /dev/stdin`/`source`/`command sh`/`env bash`/anything unrecognised). The heredoc rule is default-safe: unknown consumer ⇒ keep visible ⇒ block. Never fails open on a genuine bypass.
  - Added `rules/subrules/gh-merge-guard/merge-guard_test.sh` — 22 cases over the real script and real stdin JSON: genuine bypass merges block (plain, chained `create && merge`, `sh -c` both quote styles, `$(…)` in `-m`, backtick, and heredocs piped into `sh`/`. /dev/stdin`/`source`/`command sh`/`env bash`); legit merges and unrelated commands pass; and body / commit-message / `cat`-heredoc / `gh --body-file` text mentioning the tokens passes.
