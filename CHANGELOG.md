...
### Added
- **Dither Kit is now the default charting library for agent-authored dataviz.** The default `tech-stack` rule now tells agents to use Dither Kit for charts in rendered outputs (HTML plans, shareable visualizations, dashboards, QA/quality reports, blog visuals, and React/Next.js pages), with a new `skills/dither-kit/` reference covering install, chart selection, and when to keep ASCII/Mermaid/SVG for structural diagrams. The `docs`, `plan-render`, and `visualize` skills now route quantitative charts to Dither Kit instead of ad-hoc inline SVG or generic chart libraries.
